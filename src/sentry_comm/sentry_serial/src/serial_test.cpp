...
void callback(const geometry_msgs::Twist& cmd_vel)
{
    ...
    serial_package.angular_y = cmd_vel.angular.y;
...
int main (int argc, char** argv){
    ...
    // 创建一个Subscriber，订阅名为cmd_vel_topic的topic，注册回调函数callback 
    //ros::Subscriber sub = n.subscribe("cmd_vel", 1000, callback); 
        Serial_Package serial_package;
    serial_package.header = 0xA5;
    serial_package.linear_x = 0;
    serial_package.linear_y = 1;
    serial_package.linear_z = 0;

    serial_package.angular_z = 1;
    serial_package.angular_y = 0;
    serial_package.angular_x = 3;
